...
def join(username, password, pubkey):
    join_var = '{"join": {"username": "'+username+'","password": "'+password+'","token":"'+pubkey+'"}}'
    return join_var

def post(pub_key, message, timestamp): #fix param
    post_var = '{"token":"%s", "post": {"entry": "%s","timestamp": "%i"}}'%(pub_key, message, timestamp)
    return post_var
