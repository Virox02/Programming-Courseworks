...
def funct():
    ...
    if command == 'L':
        ...
        if len(input_list)<4:
            ...
            pro = Profile() #assigning Profile class to an object
            ...
            if yn == 'yes' and yn_2 == 'yes':
                ds_client.send(dsuserver, 3021, username, password, post_msg, bio) #sending to client with bio
            elif yn == 'no' and yn_2 == 'yes':
                ds_client.send(dsuserver, 3021, username, password, post_msg) #sending to client without bio
        ...
        if len(input_list)<2:
            ...
            if yn == 'yes':
                ...
                openweather = OpenWeather(weatherapikey, "92697", "US") #assigning OpenWeather class to an object #apikey is hardcoded at the start 
                bored = Bored() #assigning Bored class to an object. This API does not require any apikey or argument
                p2 = Profile() #assigning Profile class to an object
                ...
                if yn_2 == 'yes' and yn_3 == 'yes':
                    ds_client.send(str(p2.dsuserver), 3021, str(p2.username), str(p2.password), post_msg, bio) #seding to client with bio
                elif yn_2 == 'no' and yn_3 == 'yes':
                    ds_client.send(str(p2.dsuserver), 3021, str(p2.username), str(p2.password), post_msg) #sending to client wihtout bio
            else:
                if yn != 'no':
                    print("invalid input")
                    funct()
